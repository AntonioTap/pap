...
def obtener_compra_por_id(id):
    ...
    if compra_tupla:
        compra = {
            ...
            'gananciaPorcentaje': compra_tupla[5]
        }
        return compra
    else:
        return None

def guardar_cambios_compra(id_compra, id_producto, fechaCompra, cantidad, precioCompra, gananciaPorcentaje):
    conexion = conectar_bd()
    cursor = conexion.cursor()
    #consulta = "UPDATE compras SET id_producto = %s, fechaCompra = %s, cantidad = %s, precioCompra = %s, gananciaPorcentaje = %s WHERE id_compra = %s"
    consulta = "UPDATE compras SET id_producto = {}, fechaCompra = {}, cantidad = {}, precioCompra = {}, gananciaPorcentaje = {} WHERE id_compra = {}"
    consulta = consulta.format(id_producto, fechaCompra, cantidad, precioCompra, gananciaPorcentaje, id_compra)
    print(consulta)
...
def agregar_compra():
    if request.method == 'POST':
        ...
        precio_con_ganancia = precioCompra * (1 + (gananciaPorcentaje / 100))


        conexion = conectar_bd()
        cursor = conexion.cursor()
        consulta = "INSERT INTO compras (id_producto, fechaCompra, cantidad, precioCompra, gananciaPorcentaje) VALUES (%s, %s, %s, %s, %s)"
        cursor.execute(consulta, (id_producto, fechaCompra, cantidad, precio_con_ganancia, gananciaPorcentaje))
        conexion.commit()
        cursor.close()
        conexion.close()
        return redirect('/lista_compras')
...
def mostrar_lista_compras():
    conexion = conectar_bd()
    cursor = conexion.cursor()
    consulta = "SELECT c.id_compra, p.nombre, c.fechaCompra, c.cantidad, c.precioCompra, c.gananciaPorcentaje FROM compras c JOIN productos p ON c.id_producto = p.id_producto"
    cursor.execute(consulta)
...
def editar_compra_route(id):
    ...
    if request.method == 'POST':
        ...
        guardar_cambios_compra(id, id_producto, fechaCompra, cantidad, precioCompra, gananciaPorcentaje)
        return redirect('/lista_compras')
